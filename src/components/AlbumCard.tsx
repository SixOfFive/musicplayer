...
export default function AlbumCard({ album, sizeThreshold }: Props) {
  const nav = useNavigate();
  const play = usePlayer((s) => s.play);
  const oversized =
    sizeThreshold != null &&
    (album.bytes ?? 0) >= sizeThreshold &&
    (album.flac_count ?? 0) > 0;
      ...
      <div className="relative aspect-square w-full mb-2">
        ...
        {oversized && (
          <button
            ...
            title={`Oversized album — click to open and shrink (${album.flac_count ?? 0} FLAC)`}
          >🗜</button>
        )}
      </div>
